...
int getDuplication(const int* numbers, int length)
{
    ...
    while(end >= start)
    {
        ...
        std::cout<<"start = "<<start<<" middle = "<<middle<<" end = "<<end<<" count = "<<count<<std::endl;
        if(count == -1)
            return -1;
        if(end == start)
        {
...
void test12()
{
    int* numbers = nullptr;
    int duplications[] = { -1 };
    test("test12", numbers, 0, duplications, sizeof(duplications) / sizeof(int));
}
