...
// ====================方法3====================
void heapAdjust(vector<int>& datas, int i, int len)
{
	for(int j = 2 * i + 1; j < len; j = (2 * j + 1))
	{
		if(j + 1 < len && datas[j] < datas[j + 1])
		    j++;
		if(datas[j] > datas[i])
		{
...
void GetLeastNumbers_Solution3(const vector<int> data, int len, vector<int> &output, int k)
{
    if(k < 1 || len < k)
        return;
