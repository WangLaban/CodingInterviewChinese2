...
static void test1()
{
    ...
    test("Test1", vector<double>(input, input + sizeof(input) / sizeof(double)),
        vector<double>(output, output + sizeof(output) / sizeof(double)),  //error: cannot bind non-const lvalue reference of type 'std::vector<double>&' to an rvalue of type 'std::vector<double>' (c++中临时变量不能作为非const的引用参数) 
